  ...
  build:
    ...
    - name: 创建release标签
      ...
      run: |
        ...
        touch release.txt
        echo "功能介绍：Dockerman、定时重拨、Openclash、Bypass、Passwall、VSSR、Openvpn、nps穿透、UU加速等插件。" >> release.txt
    ...
    - name: 发布至release
      ...
      with:
        name: ${{ env.FILE_TIME }} 「 openwrt6.0-X86-64 」
    ...
    - name: 微信报告
      ...
      if: steps.organize.outputs.status == 'success' && !cancelled()
      with:
        SCKEY: ${{ secrets.SERVERCHAN }}
        text: 恭喜主人openwrt5.10最新版固件编译成功！
